"""
CQL Regular Anti-Windup — Conservative Q-Learning (offline), flow actor.
...
def norm_q(q):      # physical flow -> normalised action in [0, 1]
    return (q - Q_MIN) / (Q_MAX - Q_MIN)


def collect_offline_buffer(excel_files, buffer):
    """Roll out the anti-windup expert PI (positional PI + back-calculation) on
    each file, add Gaussian noise to the applied flow for state-action coverage,
    and store (s, a_norm, r, s2, done) transitions."""
    added = 0
    for path in excel_files:
        ...
        def obs(tout, tout_prev, int_e, q_prev, i):
            e  = tref - tout
            de = -(tout - tout_prev) / TS
            raw = np.array([tout, e, int_e, de, I_arr[i], Ta_arr[i],
                            Tin_arr[i], th_arr[i], q_prev, tref], dtype=np.float32)
        ...
        for i in range(N - 1):
            s   = obs(tout, tout_prev, int_e, q_prev, i)
            e   = tref - tout
            q_raw = KP_EXPERT * e + KI_EXPERT * int_e              # expert PI (positional)
            q_ap  = float(np.clip(q_raw, Q_MIN, Q_MAX))
            # exploration noise (normalised units) -> keep in valid flow range
            q = float(np.clip(q_ap + np.random.randn() * PREFILL_NOISE * (Q_MAX - Q_MIN),
                              Q_MIN, Q_MAX))
            a = np.array([norm_q(q)], dtype=np.float32)
            ...
            # anti-windup back-calculation with the applied (noised) flow
            int_e_next = float(np.clip(int_e + (e + KW * (q - q_raw)) * TS,
                                       -INT_E_CLIP, INT_E_CLIP))
            done = (i == N - 2)
            j    = min(i + 1, N - 1)
            s2   = obs(tout_next, tout, int_e_next, q, j) if not done else np.zeros(10, np.float32)
            buffer.add(s, a, float(r), s2, done)
            added += 1
...
def plot_curves(history, save_dir, combo_name=""):
    ...
    ax[0].set_title(f"CQL Regular Anti-Windup — critic loss{tag}"); ax[0].grid(True, alpha=0.3)
    ax[1].plot(history["epoch"], history["proxy_mae"], color="coral")
    ax[1].set_xlabel("Epoch"); ax[1].set_ylabel("Closed-loop MAE [°C]")
    ax[1].set_title(f"CQL Regular Anti-Windup — proxy MAE{tag}"); ax[1].grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(save_dir / "cql_regular_anti_windup_loss.png", dpi=150)
    plt.close(fig)
...
def main():
    ...
    print(f"CQL Regular Anti-Windup — training combo sizes {sorted(TRAIN_NFILES)} "
          f"-> {total} policies\n")

    for idx, combo in enumerate(all_combos, start=1):
        labels     = [DAY_LABELS[i] for i in combo]
        combo_name = "_".join(labels)
        save_path  = SAVE_DIR / f"cql_regular_anti_windup_{combo_name}.pt"
    ...
    best_dest = SAVE_DIR / "cql_regular_anti_windup_best.pt"
    shutil.copy(best["save_path"], best_dest)

    print("=" * 90)
    print(f"SUMMARY — {total} CQL Regular Anti-Windup policies (closed-loop MAE)")
    ...
    for n_f in range(1, 5):
        ...
        axes[1].plot(x, [r["test_rmse"] for r in sub], marker=markers[n_f],
                     color=colors[n_f], linewidth=1.5, markersize=7, label=f"{n_f} dataset(s)")
    axes[0].set_ylabel("MAE [°C]");  axes[0].set_title("CQL Regular Anti-Windup — Closed-loop MAE")
    axes[1].set_ylabel("RMSE [°C]"); axes[1].set_title("CQL Regular Anti-Windup — Closed-loop RMSE")
    for ax in axes:
        ax.set_xlabel("Policy index"); ax.grid(True, alpha=0.3); ax.legend(fontsize=7)
    fig.tight_layout()
    CHART_DIR.mkdir(parents=True, exist_ok=True)
    fig.savefig(CHART_DIR / "cql_regular_anti_windup_comparison.png", dpi=150)
    plt.close(fig)
